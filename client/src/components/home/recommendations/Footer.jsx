...
export default function Footer({ stock }) {
    ...
    const [isBuy, setIsBuy] = useState(false); // was: Buy button had no onClick at all
    ...
    return (
        <div className="text-gray-400 flex flex-row whitespace-nowrap justify-between items-end pt-3 text-sm font-light border-t border-white/20 mt-4">
            ...
            <div className="flex gap-2 items-center text-gray-300 mb-0.5">
                ...
                <button onClick={() => setIsBuy(true)} className='cursor-pointer text-green-200 pl-4 pr-4 pt-1 pb-1  font-medium rounded-lg bg-emerald-600 hover:bg-emerald-500'>Buy</button>
            </div>

            {isBuy && (
                <div className="p-4 fixed inset-0 z-50">
                    <BuyButton data={stock} type="BUY" handleIsOpen={() => setIsBuy(false)} />
                </div>
            )}
        </div>
    )
